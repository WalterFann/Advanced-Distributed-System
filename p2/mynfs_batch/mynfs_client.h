...
class MyNFS {
 public:
  MyNFS(std::shared_ptr<Channel> channel)
    ...
    int rpc_write(const char *buf, size_t size,
             off_t offset, struct fuse_file_info *fi, int ID){
      ClientContext clientContext;
  ...
  int rpc_release(struct fuse_file_info *fi){
  	ReleaseRequest request;
  	ClientContext context;
  	ErrorMessage reply;
  	request.set_fh(fi->fh);
  	Status status = stub_->mynfs_release(&context, request, &reply);
      return -reply.err();
  }

  void rpc_clear(int ID){
  	ClearRequest request;
  	ClientContext context;
  	ErrorMessage reply;
  	request.set_id(ID);
  	Status status = stub_->mynfs_clear(&context, request, &reply);
  }

  int rpc_fsync(int ID, int size){
  	FsyncRequest request;
  	FsyncReply reply;
  	ClientContext context;
  	request.set_id(ID);
  	request.set_size(size);
  	Status status = stub_->mynfs_fsync(&context, request, &reply);
  	return -reply.err();
  }
